...
- (RQOperation *)initWithRequest:(NSMutableURLRequest *)request
{
    if ((self = [self init]))
    {
        _request = request;
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    {
        if(_didFailWithErrorHandler) _didFailWithErrorHandler(_connection, _request, error);
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    _bytesReceived = 0;
    if (_responseHandler) _responseHandler(_responseReceived);
}
...
- (void)connection:(NSURLConnection *)connection didSendBodyData:(NSInteger)bytesWritten totalBytesWritten:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite
{
    if (_uploadProgressHandler)
    {
        ...
        _uploadProgressHandler(progress, totalBytesWritten, totalBytesExpectedToWrite, _responseReceived);
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    if (_accumulatedData == nil)
    {
        ...
        _accumulatedData = [[NSMutableData alloc] initWithCapacity:MAX(0, data.length)];
    }
    //[_accumulatedData appendData:data];
    [_accumulatedData setData:data];
    _bytesReceived += data.length;
    //NSLog(@"Bytes Rec %i", _bytesReceived);
    if (_downloadProgressHandler)
    {
        //NSInteger bytesTransferred = [_accumulatedData length];
        NSInteger bytesTransferred = _bytesReceived;
        NSInteger totalBytes = MAX(0, _responseReceived.expectedContentLength);
        _downloadProgressHandler((float)bytesTransferred / (float)totalBytes, bytesTransferred, totalBytes, _responseReceived);
    }
    if(_receivingHandler) _receivingHandler(_responseReceived, _accumulatedData);
}
...
- (void)addRequest:(NSMutableURLRequest *)request completionHandler:(RQCompletionHandler)completionHandler
{
...
- (void)addRequest:(NSMutableURLRequest *)request receivingHandler:(RQReceivingHandler)receivingHandler
{
    RQOperation *operation = [RQOperation operationWithRequest:request];
    operation.receivingHandler = receivingHandler;
    [self addOperation:operation];
}

- (void)addRequest:(NSMutableURLRequest *)request responseHandler:(RQResponseHandler)responseHandler
{
    RQOperation *operation = [RQOperation operationWithRequest:request];
